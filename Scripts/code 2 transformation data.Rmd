...
install.packages (c("dplyr", "ggplot2", "vcdExtra","epiDisplay", "data.table", "formattable", "table1", "haven", "devtools", "DescTools"))
```
...
**1.2. Importing of imputed SABE data base (Loading data EXCEL files)**.

```{r}
library(readxl)
biomarkers.imp <- read_excel("~/Documents/PhD Project/Data/SABE/Dataframes/biomarkers.imp.xlsx")
```

Another option for CSV files:
biomarkers.imp <- read_csv("biomarkers.imp.csv")
...
There are four demographics characteristics variables: sex, age, ethnic groups, and area of residence.
...
Age: to create vector age groups.
```{r }
biomarkers.imp$age_group <- cut( x = biomarkers.imp$age, breaks = c(60, 70, 80, Inf))

table(biomarkers.imp$age_group)

#Desc(biomarkers.imp$age_group)
#Freq(biomarkers.imp$age_group, useNA = "always")
```

Verify that each age value was assigned to the correct category by cross-tabulating the numeric and category columns. 
Cross tabulation of the numeric and category columns. 
```{r }
table("Numeric Values" = biomarkers.imp$age,   # names specified in table for clarity.
      "Categories"     = biomarkers.imp$age_group,
...
biomarkers.imp$ethnic_group <- factor(biomarkers.imp$ethnic_group,
                ...
                labels = c("indigenous", "afrocolombian","afrocolombian","white","mestize","mestize","Not reply/Don't know", "Not reply/Don't know"))
...
There are five socioeconomic position variables: 

Two of early-life SEP variables: i) family SEP and ii) Adverse Childhood Experiences ACEs.
...
Family socioeconomic position (reported before 15 year old).

```{r}
biomarkers.imp$family_sep <- factor(biomarkers.imp$family_sep,
                levels = c("1","2","3","8","9"),
                labels = c("high", "intermediate","low","Not reply/Don't know","Not reply/Don't know"))

#compute frequency tables
#Desc(biomarkers.imp$family_sep)
#Freq(biomarkers.imp$family_sep, useNA = "always")
...
biomarkers.imp$sep_occupation <- factor(biomarkers.imp$sep_occupation,
                ...
                "non-manual","non-manual","skilled manual","semi-skilled","non-manual","Not reply/Don't know","Not reply/Don't know"))
...
biomarkers.imp$CKD <- factor(biomarkers.imp$early_infection1,
                levels = c("1","2","8","9"),
                labels = c("1","0", "Not reply/Don't know","Not reply/Don't know"))
...
#Desc(biomarkers.imp$bronchitis)
#Freq(biomarkers.imp$bronchitis, useNA = "always")
```
...
Desc (biomarkers.imp$poor_health2)
```
...
biomarkers.imp$migration <- factor(biomarkers.imp$migration,
                levels = c("1","2","8","9"),
                labels = c("1","0", "0","0"))

Desc(biomarkers.imp$migration)
...
Freq(biomarkers.imp$neglected_food, useNA = "always")
...
Then, we create new combination of vectors not present in the data (adverse childhood experiences). First step, it is calculate a new variable called aces (sum of six exposures reported). Second step, it was recoded into three main categories. 

Levels: a) none  b) one or two  c) three  d) four or more adverse childhood exposures

```{r}
#class(biomarkers.imp$early_infection)
#class(biomarkers.imp$migration)
#class(biomarkers.imp$emotional_abuse)
#class(biomarkers.imp$early_infection)
...
biomarkers.imp$early_infection    <- as.numeric(as.character(biomarkers.imp$early_infection))

biomarkers.imp$acesum <- biomarkers.imp$neglected_food + biomarkers.imp$household_violence + biomarkers.imp$migration + biomarkers.imp$emotional_abuse + biomarkers.imp$poor_health2 + biomarkers.imp$early_infection   

biomarkers.imp$acesum <- factor(biomarkers.imp$acesum,
                levels = c("1","2","3","4","5","6"),
                labels = c("one or two", "one or two", "three","four or more", "four or more", "four or more"))

#compute frequency tables
Desc(biomarkers.imp$acesum)
```

Summary childhood experiences of study population by sex and age groups.

```{r Table 4. Description Adverse Childhood Experiences by sex and age groups}
...
biomarkers.imp$poor_health2       <- as.factor(as.numeric(biomarkers.imp$poor_health2))
biomarkers.imp$early_infection    <- as.factor(as.numeric(biomarkers.imp$early_infection))

table_4 <-table1(~neglected_food + household_violence + migration + emotional_abuse + poor_health2 + early_infection + acesum| sex + age_group, data=biomarkers.imp)
...
table_6 <-table1(~neglected_food + household_violence + migration + emotional_abuse + poor_health2 + early_infection + acesum| ethnic_group, data=biomarkers.imp)
...
biomarkers.imp$comorbidities <- factor(biomarkers.imp$comorbidities,
                levels = c("0","1","2","3"),
                labels = c("none","one", "two or more","two or more"))
...
table_19 <- table1(~hta + cvd + diabetes + comorbidities + medicationsum + smoking + alcohol + BMI + wc + SBP + DBP | sex + educational_level, data=biomarkers.imp)
...
```{r }
writexl::write_xlsx( x = biomarkers.imp, path =  '/Users/macbookpro/Documents/PhD Project/Data/SABE/dataframes/biomarkers.imp2.xlsx' )
```
...
```{r }
writexl::write_xlsx( x = covbiomarkers, path =  '/Users/macbookpro/Documents/PhD Project/Data/SABE/dataframes/covbiomarkers.xlsx' )
```
